...
export class BoardComponent implements OnInit {
  ...
  handleRealtimeUpdates() {
    const tableChanges = this.dataService.getTableChanges();

    tableChanges.changes.subscribe((update: any) => {
      ...
      switch (update.table) {
        case 'cards':
          this.handleCardUpdate(record, event);
          break;
        case 'lists':
          if (event === 'INSERT') {
  ...
  private handleCardUpdate(record: any, event: string) {
    if (event === 'INSERT') {
